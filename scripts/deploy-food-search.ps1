...
Write-Host 'Deploying food-search with Supabase server-side bundling...'
& $supabase functions deploy food-search --project-ref $ProjectRef --use-api
if ($LASTEXITCODE -ne 0) { throw 'The food-search Edge Function did not deploy.' }

Write-Host 'Verifying the remote Edge Function list...'
& $supabase functions list --project-ref $ProjectRef
if ($LASTEXITCODE -ne 0) { throw 'food-search was sent for deployment, but the remote list could not be verified.' }

Write-Host 'Food search deployed. FatSecret remains off unless its licensed server secrets are configured.'
